import { Resource, IResource, Tag, Stack, ArnFormat } from 'aws-cdk-lib';
import { CfnExperiment } from 'aws-cdk-lib/aws-evidently';
...
export class Experiment extends ExperimentBase {
  /**
	 ...
	 * @returns IExperiment
	 */
  public static fromExperimentAttributes(
    scope: Construct,
    id: string,
    ...
    class ImportedExperiment extends ExperimentBase {
      ...
      public readonly experimentName = Stack.of(scope).splitArn(
        attrs.experimentArn,
  ...
  constructor(scope: Construct, id: string, props: ExperimentProps) {
    ...
    if (metricGoals.length > 3) {
      throw new Error(
        'You are specifying more than the supported number of MetricGoals for an experiment.',
      );
    }
...
export class OnlineAbConfig {
  ...
  public _renderOnlineAbConfig(): CfnExperiment.OnlineAbConfigObjectProperty {
    return {
      ...
      treatmentWeights: this.treatmentWeights?.map((weight) =>
        weight._renderWeight(),
      ),
    };
  }
}
