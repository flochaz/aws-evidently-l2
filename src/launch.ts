...
export class Launch extends LaunchBase {
  ...
  /**
	 ...
	 * @returns ILaunch
	 */
  public static fromLaunchAttributes(
    scope: Construct,
    id: string,
    ...
    class ImportedLaunch extends LaunchBase {
      ...
      public readonly launchName = Stack.of(scope).splitArn(
        attrs.launchArn,
  ...
  constructor(scope: Construct, id: string, props: LaunchProps) {
    ...
    if (groups.length > 5) {
      throw new Error(
        `A launch can have up to five launch groups. You have specified ${groups.length} groups.`,
      );
    }
