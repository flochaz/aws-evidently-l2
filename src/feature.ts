...
export class Feature extends FeatureBase {
  /**
	 ...
	 * @returns IFeature
	 */
  public static fromFeatureAttributes(
    scope: Construct,
    id: string,
    ...
    class ImportedFeature extends FeatureBase {
      ...
      public readonly featureName = Stack.of(scope).splitArn(
        attrs.featureArn,
  ...
  constructor(scope: Construct, id: string, props: FeatureProps) {
    ...
    if (variations.length < 1) {
      throw new Error(
        `You must specify at least one variation for feature ${props.featureName}`,
      );
    }
    ...
    this.resource = new CfnFeature(this, 'Resource', {
      ...
      variations: props.variations.map((variation) =>
        variation._renderVariation(),
      ),
